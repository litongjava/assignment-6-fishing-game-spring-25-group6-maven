...
public class Ama_amaTest {
	...
	/** test isLegalSize. */
	@Test
	public void isLegalSizeTest() {
		Ama_ama a = new Ama_ama();
		double len = a.getLength();
		if (len < 11) {
			Assert.assertEquals("This fish is " + len + " it must be >= 11 to be legal size", false,
					a.isLegalSize());
		} else {
			Assert.assertEquals("This fish is " + len + " which is >= 11, the legal size", true,
					a.isLegalSize());
		}
	}
	...
	@Test
	public void getCatchMethodsTest() {
		Ama_ama a = new Ama_ama();
		...
		for (int i = 0; i < validCatchMeth.length; i++) {
			...
			Assert.assertTrue(a.getName() + "is missing " + validCatchMeth[i] + "as a catch method",
					match);
			match = false;
		}
