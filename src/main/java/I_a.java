...
public abstract class I_a implements Comparable<I_a> {
	...
	/**
	 * constructor. initial length should be generated in subclass.
	 * 
	 ...
	 *                           minLength
	 **/
	public I_a(String name, String englishName, String scientificName, double maxLength,
			double minLength, double length, double weight, String[] diet, String bodyColor,
			String finColor, String sex) throws FishSizeException {
		...
		if (length >= minLength && length < maxLength) {
			...
			throw new FishSizeException(name + " length must be within " + minLength + " and "
					+ maxLength + " inches long. But was sent " + length);
		}
		this.weight = weight;
		this.diet = diet;
	...
	/**
	 * returns new I_a of next level When a fish reaches maxSize, it should
	 * transform to next level Hawaiian type.
	 * 
	...
	/**
	 * initialize random fish sex choose randomly male or female used by some
	 * subclasses.
	 */
