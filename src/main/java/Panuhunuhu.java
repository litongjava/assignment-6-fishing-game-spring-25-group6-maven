...
/**
 * Panuhunuhu class (juvenile parrotfish). Second stage of Parrotfish cycle
 * 
 * @author Mika Evans
 * @since 2/20/25
 */
public class Panuhunuhu extends Ohua {
	...
	/**
	 * Constructor for making Panuhunuhu with random length. Creates basic fish
	 * properties Initializes random length and determines sex
	 */

	public Panuhunuhu() {
		super("Panuhunuhu", MAX_LENGTH, MIN_LENGTH, MIN_LENGTH, (MIN_LENGTH * 2), DIET_ITEMS, "gray",
				"reddish gray", "none");
		this.initLen();
		this.initSex();
	}
	...
	public Panuhunuhu(double length) {
		super("Panuhunuhu", MAX_LENGTH, MIN_LENGTH, length, (length * 2), DIET_ITEMS, "gray",
				"reddish gray", "none");
		if (length > MAX_LENGTH || length < MIN_LENGTH) {
			throw new FishSizeException(
					"Panuhunuhu must be between " + MIN_LENGTH + " and " + MAX_LENGTH + " inches long.");
		}
		this.initSex();
	}
	...
	/**
	 * Levels up to Uhu when fish outgrows juvenile stage. Maintains sex through
	 * growth
	 * 
